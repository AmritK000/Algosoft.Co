  ...
  var b = function(a, b) {
    ...
    window.devicePixelRatio > 1 &&
      ((f = window.devicePixelRatio), (d.style.width = d.style.height = [
        b.size,
        "px"
      ].join("")), (d.width = d.height = b.size * f), e.scale(
        f,
        f
      )), e.translate(b.size / 2, b.size / 2), e.rotate(
      (-0.5 + b.rotate / 180) * Math.PI
    );
    var g = (b.size - b.lineWidth) / 2;
    b.scaleColor && b.scaleLength && (g -= b.scaleLength + 2), (Date.now =
      Date.now ||
      function() {
        return +new Date();
      });
    var h = function(a, b, c) {
      ...
      e.beginPath(), e.arc(
        ...
        2 * Math.PI * c,
        d
      ), (e.strokeStyle = a), (e.lineWidth = b), e.stroke();
    },
      i = function() {
        var a, c;
        ...
        for (var d = 24; d > 0; --d)
          d % 6 === 0
            ? ((c = b.scaleLength), (a = 0))
            : ((c = 0.6 * b.scaleLength), (a = b.scaleLength - c)), e.fillRect(
            -b.size / 2 + a,
            0,
            c,
            1
...
function counter(targetNumber, duration) {
  var currentNum = 0, beatDuration = duration / targetNumber, $this = $(this);

  var interval = setInterval(function() {
    $this.text(++currentNum);

    if (currentNum >= targetNumber) clearInterval(interval);
  }, beatDuration);
}

function easyPieChart(i) {
  if ($(".chart").length < i - 1) return;
